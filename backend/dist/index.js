...
const PomodoroSession = (0, mongoose_1.model)('PomodoroSession', PomodoroSessionSchema);
...
const GoogleAuth = (0, mongoose_1.model)('GoogleAuth', GoogleAuthSchema);
// Weekly Quests
const WeeklyQuestSchema = new mongoose_1.Schema({
    userId: { type: String, index: true },
    title: { type: String, required: true },
    target: { type: Number, default: 1 },
...
const WeeklyQuest = (0, mongoose_1.model)('WeeklyQuest', WeeklyQuestSchema);
// Auth
...
app.get('/notes/notebooks', async (req, res) => {
    const userId = String(req.userId);
...
app.post('/notes/:id/pin', async (req, res) => {
    const note = await Note.findByIdAndUpdate(req.params.id, { isPinned: true }, { new: true });
    res.json(note);
});
app.post('/notes/:id/unpin', async (req, res) => {
    const note = await Note.findByIdAndUpdate(req.params.id, { isPinned: false }, { new: true });
    res.json(note);
});
app.post('/notes/:id/archive', async (req, res) => {
    const note = await Note.findByIdAndUpdate(req.params.id, { isArchived: true }, { new: true });
    res.json(note);
});
app.post('/notes/:id/unarchive', async (req, res) => {
    const note = await Note.findByIdAndUpdate(req.params.id, { isArchived: false }, { new: true });
    res.json(note);
});
...
app.get('/notes/search/advanced', async (req, res) => {
    const userId = String(req.userId);
...
app.get('/integrations/google/auth-url', async (req, res) => {
...
app.post('/integrations/google/oauth/callback', async (req, res) => {
    const { code, redirectUri } = req.body || {};
    const clientId = process.env.GOOGLE_CLIENT_ID;
    const clientSecret = process.env.GOOGLE_CLIENT_SECRET;
    const finalRedirect = redirectUri || process.env.GOOGLE_REDIRECT_URI;
    if (!code || !clientId || !clientSecret || !finalRedirect)
        return res.status(400).json({ error: 'missing oauth config or code' });
    try {
        ...
        await GoogleAuth.findOneAndUpdate({ userId: String(req.userId) }, { accessToken: json.access_token, refreshToken: json.refresh_token, expiryDate, scope: json.scope }, { upsert: true, new: true });
        res.json({ ok: true });
    }
    catch (e) {
...
app.get('/integrations/google/events', async (req, res) => {
    try {
        ...
        const events = (json.items || []).map((it) => ({ id: it.id, title: it.summary, start: it.start?.dateTime || it.start?.date, end: it.end?.dateTime || it.end?.date, location: it.location }));
        res.json({ events });
    }
    catch (e) {
...
// Outlook (stub)
app.get('/integrations/outlook/auth-url', async (_req, res) => {
    res.json({ url: 'https://login.microsoftonline.com/common/oauth2/v2.0/authorize?...' });
});
app.post('/integrations/outlook/import', async (_req, res) => {
    res.json({ imported: 3, status: 'ok' });
});
// Trello / CSV import (stub)
app.post('/integrations/trello/import', async (req, res) => {
...
app.get('/gamification/quests', async (req, res) => {
    const userId = String(req.userId);
...
app.post('/gamification/quests/:id/progress', async (req, res) => {
    const userId = String(req.userId);
...
app.post('/gamification/quests/refresh', async (req, res) => {
    const userId = String(req.userId);
    const weekStart = getWeekStart();
...
app.post('/ai/predict-duration', async (req, res) => {
    const { task } = req.body || {};
    ...
    const avg = hist.length ? Math.round(hist.reduce((s, t) => s + (t.durationMin || 30), 0) / hist.length) : 30;
...
// Digital health
app.get('/health/digital', async (req, res) => {
...
app.post('/ai/best-slot', async (req, res) => {
...
app.post('/billing/cancel', (req, res) => {
    ...
    if (plan !== 'annual')
        return res.json({ cancelled: true, refund: 0 });
...
function parseVoiceCommand(text) {
    ...
    // Intents existentes
    if (t.includes('iniciar foco') || t.includes('start focus'))
    ...
    if (t.includes('próximo evento') || t.includes('next event'))
        return { intent: 'next_event' };
    ...
    if (t.includes('iniciar pausa') || t.includes('start break'))
        return { intent: 'start_break' };
    if (t.includes('próxima tarefa') || t.includes('next task'))
    ...
    if (t.includes('pausar pomodoro') || t.includes('pause pomodoro'))
        return { intent: 'pause_pomodoro' };
    if (t.includes('resumir pomodoro') || t.includes('resume pomodoro'))
        return { intent: 'resume_pomodoro' };
    if (t.includes('finalizar pomodoro') || t.includes('finish pomodoro'))
        return { intent: 'finish_pomodoro' };
    return { intent: 'unknown' };
}
app.post('/assistant/voice/command', async (req, res) => {
...
// Alexa webhook (ASK)
app.post('/integrations/alexa/webhook', async (req, res) => {
...
app.post('/integrations/google-assistant/webhook', async (req, res) => {
...
app.get('/integrations/siri/shortcut', (req, res) => {
