...
export const useAssistant = create<AssistantStore>((set, get) => ({
  ...
  optimizeWeeklyPlan: async (planId: string) => {
    ...
    try {
      ...
      // Usar ML service para otimização real
      const mlRequest: MLOptimizationRequest = {
        tasks: originalPlan.tasks.map(task => mlOptimizationService.convertToMLTask(task)),
        events: originalPlan.events.map(event => mlOptimizationService.convertToMLEvent(event)),
        userPreferences: mlOptimizationService.getDefaultUserPreferences(),
        constraints: {
          startDate: originalPlan.weekStart.toISOString(),
          endDate: originalPlan.weekEnd.toISOString(),
          workingDays: [1, 2, 3, 4, 5], // Segunda a Sexta
        },
      };
      
      const mlResult = await mlOptimizationService.optimizeSchedule(mlRequest);
      
      // Criar plano otimizado baseado no resultado do ML
      const optimizedPlan: WeeklyPlan = {
        ...originalPlan,
        tasks: originalPlan.tasks.map(task => {
          const optimizedTask = mlResult.optimizedSchedule.find(s => s.taskId === task.id);
          return optimizedTask ? {
            ...task,
            suggestedTime: new Date(optimizedTask.suggestedStartTime),
          } : task;
        }),
        score: mlResult.overallScore,
        insights: [
          ...originalPlan.insights,
          ...mlResult.insights.map(insight => insight.message),
        ],
        isOptimized: true,
        updatedAt: new Date(),
      };
      
      const changes = mlResult.conflicts.map(conflict => ({
        type: 'reschedule' as const,
        itemId: 'auto-generated',
        description: conflict.description,
        impact: conflict.severity === 'high' ? 'positive' : 
                conflict.severity === 'medium' ? 'neutral' : 'negative',
      })) as {
        type: 'reschedule' | 'add' | 'remove' | 'modify';
        itemId: string;
        description: string;
      ...
      const result: OptimizationResult = {
        ...
        optimizedPlan,
        changes,
        scoreImprovement: mlResult.improvement,
        insights: mlResult.insights.map(insight => insight.message),
      };
      ...
      mlResult.insights.forEach(insight => {
        get().addInsight({
          type: 'optimization',
          title: 'Otimização ML Aplicada',
          ...
          action: {
            type: 'optimize',
            suggestion: 'Aplicar sugestões de otimização',
          },
          isRead: false,
        });
      });
